...
// Test: inline constant (no intermediate source global) with two encodings.
...
// This pattern is common in real-world models (e.g., LLaMA 405B).
...
// CHECK-LABEL: module @inline_constant_with_parameter_attr
//       CHECK:   util.global private @[[$DEVICE_A:.+]] =
module @inline_constant_with_parameter_attr {
  util.global private @device_a = #device_target_local
  // Note: no source global here - constant is inline.
  util.global private @encoded_v1 : !stream.resource<constant>
  util.global private @encoded_v2 : !stream.resource<constant>

  // CHECK: util.initializer
  util.initializer {
    // The constant is created inline, not loaded from a global.
