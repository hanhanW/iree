...
struct SourceTraceResult {
  // The source global, if found (may be null for inline constants).
...
public:
  ...
  SourceGlobalInfo getSourceInfo(StringRef key) {
    return sourceGlobals.find(key)->second;
  }

  // Returns the unified encoding for the given source key, or std::nullopt if
  // not found.
...
LogicalResult GlobalEncodingAnalyzer::collectGlobalEncodings() {
  ...
  for (auto initOp : moduleOp.getOps<IREE::Util::InitializerOp>()) {
    initOp.walk([&](IREE::Stream::TensorEncodeOp encodeOp) {
      ...
      if (sourceKey.empty()) {
        LDBG() << "    Skipping: empty source key";
        return;
      }
      LDBG() << "    Source key: " << sourceKey;
    ...
    bool isValid =
        llvm::TypeSwitch<Operation *, bool>(defOp)
            .Case([&](IREE::Util::GlobalLoadOpInterface loadOp) {
              ...
              // It is okay to trace through globalOp because data is not
              // mutable. I.e., there are no tensor dispatch ops in the chain.
