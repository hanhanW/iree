...
SmallVector<int64_t> getIntrinsicVectorSize(IREE::GPU::MMAAttr mma,
                                            int64_t roleIdx) {
  // TODO: Query the value from GPU attributes.
...
enumerateMatmulIntrinsic(TypeRange elementTypes, IREE::GPU::TargetAttr target) {
...
gpuMaterializeEncodingForTarget(RankedTensorType tensorType,
  ...
  std::optional<IREE::GPU::MMAAttr> mma =
      enumerateMatmulIntrinsic(elementTypes, gpuTargetAttr);
