  ...
  auto bufferMappingFn = [&](Operation *op) -> WalkResult {
    return TypeSwitch<Operation *, LogicalResult>(op)
        ...
        .Case<linalg::LinalgOp>([&](linalg::LinalgOp linalgOp) {
          return analyseDPSOps(
              cast<DestinationStyleOpInterface>(linalgOp.getOperation()), plan);
        })
        .Case<IREE::LinalgExt::LinalgExtOp>(
            [&](IREE::LinalgExt::LinalgExtOp linalgExtOp) {
              return analyseDPSOps(
                  cast<DestinationStyleOpInterface>(linalgExtOp.getOperation()),
                  plan);
            })
        .Case<tensor::PackOp>([&](tensor::PackOp op) {
          return analyseDPSOps(
              cast<DestinationStyleOpInterface>(op.getOperation()), plan);
        })
