...
llvm::raw_ostream &breakLog() { return llvm::errs(); }
...
[[noreturn]] void exitWithUserError(const llvm::Twine &msg) {
...
void warnOnUnknownPasses(const llvm::StringSet<> &names,
...
// in reparseAndReplace still targets the specific live op identified by
...
LogicalResult writeOpToFile(Operation *op, llvm::StringRef path) {
  std::error_code ec;
...
//
// Needed because leaf symbol names aren't unique: IREE pipelines routinely
// produce multiple `hal.executable.variant @<target>` ops with the same
// leaf name in different parent `hal.executable`s, and a leaf-only match
// would splice into the wrong variant.
std::string buildSymbolPath(Operation *op) {
...
Operation *findMatchingOp(ModuleOp parsedRoot, Operation *liveOp) {
  ...
  if (anyMatch && !liveSym.empty()) {
    breakLog() << "[iree-debug-break] note: live op @" << liveSym
               << " not found in parsed dump; falling back to first "
               << opName
...
// Re-parses |path| as a ModuleOp in |liveOp|'s context, finds the op in the
...
LogicalResult reparseAndReplace(Operation *liveOp, llvm::StringRef path) {
...
BreakSignal waitFileSentinels(llvm::StringRef breakFile,
                              unsigned pollIntervalMs, unsigned timeoutSec) {
...
PassDebugBreakInstrumentation::PassDebugBreakInstrumentation(
  ...
  // lifetime of this instrumentation so parallelized nested-op pipelines
  // (e.g. per-hal.executable.variant codegen) don't race on stdin and
  // the exchange file. The destructor restores the prior state so a
  // long-lived session (e.g. Python bindings) that reuses the context
  // for later non-debug compiles stays multithreaded.
...
void PassDebugBreakInstrumentation::breakHere(Pass *pass, Operation *op,
  ...
  if (!patchFile.empty()) {
    breakLog() << "[iree-debug-break] auto-patching from " << patchFile
               << "\n";
    breakLog().flush();
    if (failed(reparseAndReplace(op, patchFile))) {
  ...
  while (true) {
    ...
    if (sig == BreakSignal::Abort) {
      exitWithUserError(llvm::Twine("aborted ") + phase + " pass '" +
                        argument + "'");
    }
    if (sig == BreakSignal::Unrecognized) {
      continue;
    }

    if (succeeded(reparseAndReplace(op, breakFile))) {
      break;
    }
    if (mode == DebugBreakMode::File) {
...
createPassDebugBreakInstrumentationFromFlags(MLIRContext *context) {
  ...
  return std::make_unique<PassDebugBreakInstrumentation>(
      ...
      clTestPatchFile, clBreakMode, clBreakPollIntervalMs,
      clBreakTimeoutSec);
}
