...
std::unique_ptr<PassManager> Invocation::createPassManager() {
  ...
  if (auto breakInstrumentation =
          createPassDebugBreakInstrumentationFromFlags(
              passManager->getContext())) {
    // The instrumentation's ctor/dtor toggles MLIRContext multithreading
    // for its own lifetime, so per-variant codegen doesn't race on stdin
    // or the exchange file while a break is active. Scoping lives there
    // so long-lived sessions (e.g. Python bindings) that reuse the
    // context aren't silently left single-threaded after this invocation.
