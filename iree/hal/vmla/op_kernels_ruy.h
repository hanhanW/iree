...
Status MatMul::Execute(RuntimeState* runtime_state,
  ...
  spec.set_bias(buffers.bias_buffer.data());

  if (buffers.multiplier_mantissa_buffer.size() == 1) {
    spec.set_multiplier_fixedpoint(buffers.multiplier_mantissa_buffer[0]);
    spec.set_multiplier_exponent(buffers.multiplier_exponent_buffer[0]);
  } else {
    spec.set_multiplier_fixedpoint_perchannel(
        buffers.multiplier_mantissa_buffer.data());
    spec.set_multiplier_exponent_perchannel(
        buffers.multiplier_exponent_buffer.data());
  }

  ruy::Mul(lhs, rhs, spec, &runtime_state->context, &dst);
