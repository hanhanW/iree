...
// Function-scope round-trip: canonicalize is a function-scope pass, so
// the break fires once per function with the live op identified in the
// banner (op=util.func @<sym>). The pipeline runs canonicalize multiple
// times across the function nest, so we feed a bounded stream of
// continues sized comfortably above the observed count. The final
// pipeline completes and produces the same ROUNDTRIP IR as the no-edit
// module-scope tests.
// RUN: for i in $(seq 16); do echo continue; done | iree-compile %s \
// RUN:     --iree-hal-target-device=local \
// RUN:     --iree-hal-local-target-device-backends=llvm-cpu \
// RUN:     --compile-to=abi \
// RUN:     --iree-debug-break-after=canonicalize \
// RUN:     --iree-debug-break-file=%t-fn.mlir \
// RUN:     2> %t-fn.stderr \
// RUN:   | FileCheck %s --check-prefix=ROUNDTRIP
// RUN: FileCheck --check-prefix=BANNER-FN %s < %t-fn.stderr

// Verify the banner reports a function-scope op, not a module — that's
// the new behavior. Any of the per-function fires satisfies the check.
// BANNER-FN: [iree-debug-break] BEGIN phase=after pass=canonicalize op=util.func @
// BANNER-FN: [iree-debug-break] END phase=after pass=canonicalize
